...
class Gui(tk.Tk):
    ...
    def build_parameter_menu(self):
        self.horizontal_slider_1 = tk.Scale(self, length=1000, from_=0, to=1000, orient=tk.HORIZONTAL,
                                            label="Genotype Pool Size", resolution=2)
        self.horizontal_slider_1.set(GENOTYPE_POOL_SIZE)
        self.horizontal_slider_1.pack()
        self.horizontal_slider_2 = tk.Scale(self, length=1000, from_=0, to=1000, orient=tk.HORIZONTAL,
                                            label="Adult Pool Size")
        self.horizontal_slider_2.set(ADULT_POOL_SIZE)
        self.horizontal_slider_2.pack()
        self.horizontal_slider_3 = tk.Scale(self, length=1000, from_=0, to=300, orient=tk.HORIZONTAL,
                                            label="Genotype Length")
        self.horizontal_slider_3.set(GENOTYPE_LENGTH)
        self.horizontal_slider_3.pack()
        self.horizontal_slider_4 = tk.Scale(self, length=1000, from_=0.0, to=1.0, resolution=-1, orient=tk.HORIZONTAL,
        ...
        self.horizontal_slider_7 = tk.Scale(self, length=1000, from_=0, to=100, orient=tk.HORIZONTAL,
        ...
        self.horizontal_slider_9 = tk.Scale(self, length=1000, from_=0, to=40, orient=tk.HORIZONTAL,
                                            label="Symbol Set Size")
        self.horizontal_slider_9.set(SYMBOL_SET_SIZE)
        self.horizontal_slider_9.pack()
        self.horizontal_slider_10 = tk.Scale(self, length=1000, from_=0, to=10000, orient=tk.HORIZONTAL,
                                             label="Max Number of Generations")
        self.horizontal_slider_10.set(self.generations)
        ...
        tk.Radiobutton(radiogroup1, text="Full", variable=self.adult_selection_protocol, value=1).pack(side=tk.LEFT)
        tk.Radiobutton(radiogroup1, text="Over Production", variable=self.adult_selection_protocol, value=2).pack(side=tk.LEFT)
        ...
        tk.Radiobutton(radiogroup2, text="Fitness Proportionate", variable=self.radio_2_value, value=1).pack(side=tk.LEFT)
        tk.Radiobutton(radiogroup2, text="Sigma-scaling", variable=self.radio_2_value, value=2).pack(side=tk.LEFT)
        tk.Radiobutton(radiogroup2, text="Tournament selection", variable=self.radio_2_value, value=3).pack(side=tk.LEFT)
        tk.Radiobutton(radiogroup2, text="4th Selection Protocol", variable=self.radio_2_value, value=4).pack(side=tk.LEFT)
        radiogroup2.grid(row=4, column=0)
        radiogroup2.pack(anchor=tk.W)

        self.radio_3_value = tk.IntVar()
        self.radio_3_value.set(PROBLEM)
        radiogroup3 = tk.Frame(self)
        tk.Label(radiogroup3, text="Problem type").pack(anchor=tk.W)
        tk.Radiobutton(radiogroup3, text="One Max", variable=self.radio_3_value, value=1).pack(side=tk.LEFT)
        tk.Radiobutton(radiogroup3, text="LOLZ Prefix", variable=self.radio_3_value, value=2).pack(side=tk.LEFT)
        tk.Radiobutton(radiogroup3, text="Surprising Sequence Local", variable=self.radio_3_value, value=3).pack(side=tk.LEFT)
        tk.Radiobutton(radiogroup3, text="Surprising Sequence Global", variable=self.radio_3_value, value=4).pack(side=tk.LEFT)
    ...
    def start_EA(self):
        ...
        self.population.set_parameters(genotype_pool_size=self.horizontal_slider_1.get(),
                                       ...
                                       problem=self.radio_3_value.get()
                                       )
        self.population.initialize_genotypes()
        self.generations = self.horizontal_slider_10.get()
        self.current_generation = 0
        self.fitness_log_average = []
        self.fitness_log_best = []
        self.run_EA()
    ...
    def run_EA(self):
        ...
        if self.current_generation < self.horizontal_slider_10.get() and \
